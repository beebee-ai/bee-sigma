...
function ClientLogo({ name, logo }: { name: string, logo?: string }) {
  ...
  if (!logo || error) {
    return (
      <div className="flex items-center justify-center whitespace-nowrap group/item cursor-default">
        <span className="text-lg md:text-xl font-bold text-slate-400 group-hover/item:text-gold-600 transition-colors duration-300">
          {name}
        </span>
      </div>
    )
  ...
  return (
    <div className="flex items-center justify-center opacity-70 hover:opacity-100 transition-opacity duration-300 grayscale hover:grayscale-0 whitespace-nowrap">
      <img
        src={logo}
        alt={name}
